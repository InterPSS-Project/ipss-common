...
public class DStab_IEEE9Bus_Test2 {
	...
	public void test_IEEE9Bus_Dstab(){
		...
		BlockingQueue<Double> dataQueue = new LinkedBlockingQueue<Double>();
		BlockingQueue<Double> updateQueue = new LinkedBlockingQueue<Double>();
		IpssLogger.getLogger().setLevel(Level.FINE);
		
		if (dstabAlgo.initialization()) {
			//run step by step, this provides users more flexibility to make changes to the network 
			Thread ipssThread = new Thread(() -> {
				for (double t = 0; t < dstabAlgo.getTotalSimuTimeSec(); t += dstabAlgo.getSimuStepSec()) {
					...
					try {
						...
						//TODO get the data
						dataQueue.put(data);
						double parameter = updateQueue.take();
				...
				try {
					dataQueue.put(Double.MIN_VALUE);
				} catch (InterruptedException e) {
					e.printStackTrace();
				}
			});
				...
				while (active) {
					try {
						...
						if (data == Double.MIN_VALUE) {
							...
							double parameter = 0;
							//TODO calculate the parameter depend on the data
							updateQueue.put(parameter);
						}
					} catch (InterruptedException e) {
						e.printStackTrace();
					}
